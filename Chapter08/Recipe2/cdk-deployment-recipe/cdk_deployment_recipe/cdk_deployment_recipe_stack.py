...
class CdkDeploymentRecipeStack(Stack):
  def __init__(self, scope: Construct, 
    ...
    # As many stages as needed, these are just templates
    pipeline.add_stage(GlueStage(self, "integration", 
        env={"region":"<integration region>","account":"<integration account>"}))

    pipeline.add_stage(GlueStage(self, "prod", 
        env={"region":"<prod region>","account":"prod account"}))

    pipeline.add_stage(GlueStage(self, "prod2", 
        env={"region":"<prod region2>","account":"prod account2"}))        
